...
const ModalABM: React.FC<ModalNotaProps> = ({ modalAbierto, handleCancelar, handleAgregar, valoresIniciales }) => {
    ...
    if (valoresIniciales) valoresIniciales.fecha = dayjs(valoresIniciales.fecha);

    return (
        <Modal title="Agregar Nota" open={modalAbierto} onOk={manejarEnvio} onCancel={handleCancelar}>
            <Form form={formulario} layout="vertical" initialValues={valoresIniciales}>
                <Item label="Fecha" name="fecha" rules={[{ required: true, message: "Seleccione una fecha" }]}>
                    <DatePicker showTime format="DD-MM-YYYY HH:mm:ss" />
                </Item>
                <Item label="Nota" name="nota" rules={[{ required: true, message: "Ingrese una nota" }]}>
