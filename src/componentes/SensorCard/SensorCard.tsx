...
const SensorCard: React.FC<TablaCrudProps> = ({
  ...
  if (isPileta) {
    config = {
      autoFit: true,
      autoResize: true,
      percent: parseFloat((ultimoValor / canal.maximo).toFixed(2)),
      style: {
        outlineBorder: 4,
        outlineDistance: 2,
        waveLength: 40,
    ...
    config = {
      ...
      data: {
        target: ultimoValor,
        total: canal.maximo,
        thresholds: [canal.maximo * 0.8, canal.maximo * 0.9, canal.maximo],
      },
      scale: {
        color: {
    ...
    config = {
      percent: ultimoValor / canal.maximo,
      autoFit: true,
      autoResize: true,
      color: ["#E8EFF5", "#66AFF4"],
      annotations: [
  ...
  return (
    <Card
      loading={cargando}
      actions={[
        <Text type="secondary">Mínimo {canal.minimo}</Text>,
        <Text type="secondary">Máximo {canal.maximo}</Text>,
